...
v3.8.3 采用开源源码正式发布口径。candidate-2 已通过 macOS 8A 与 8B，覆盖真实 Skill、分页、安装升级、回退和数据保留验证。macOS ARM 安装包作为未签名 Preview 提供，Windows 安装包只保留 CI 构建结果且不声明人工验收完成。进度见 `CURRENT-PLAN.md` 与 `docs/current/operations/RELEASE-READINESS.md`。
