...
顶部主导航的新增、删除或职责变化属于产品信息架构变化，需要用户确认，并同步 `AGENTS.md` 与 `docs/current/product/UI-SPECIFICATION.md`。
...
详细基准见 `docs/current/product/UI-SPECIFICATION.md`。
...
v3.8.3 candidate-2 已通过 macOS 8A 与 8B，包含真实 Skill、分页、安装升级、回退和数据保留验证，作为当前内部开发基线。v3.8.2 仍为最新正式版本。macOS 正式发布继续等待 Developer ID 签名、公证和 Gatekeeper 验证，Windows 候选继续延期。进度见 `CURRENT-PLAN.md` 与 `docs/current/operations/RELEASE-READINESS.md`。
