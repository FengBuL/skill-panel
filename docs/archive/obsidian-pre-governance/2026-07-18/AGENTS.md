...
- `delivery.md` 副本：修改范围、测试、截图、数据安全和回退。
- `DEVELOPMENT-LOG.md`：完成批次、commit、验证、证据和回退点。

产品需求变化时更新 `PRD.md`，UI 决策变化时更新 `UI-SPECIFICATION.md`，发布状态变化时更新 `RELEASE-READINESS.md`。截图、测试报告和安装包保存在代码项目中，台账只记录路径与 commit。

## 规则变更记录

### 2026-07-13 · DOC-NAV-01

- 影响章节：产品定位与功能优先级。
- 旧规则摘要：Library 是主入口，Editor 服务已有 Skill 维护，未明确顶部导航清单和任务页归属。
- 新规则摘要：顶部固定五个主导航；Detail、Editor、Create、AI Assistant、Preview 归属 Library；Editor 需要已选中的 Skill。
- 修改原因：统一 PRD、UI 规范、现有导航实现和后续原型提示词，消除页面清单与顶部导航清单的混淆。
- 影响范围：产品文档、UI 原型、应用壳模块文档、Editor 进入路径和视觉验收。
- 分支与 commit：`codex/agent-codex-v3.8`；规则依据为稳定 HEAD `ec58c8deb2dcaeefc845d3762d7af13e6ed87590`，本次文档修改尚未提交。
- 状态：已验证；依据为当前 TopBar、AppShell、Detail 进入路径和 v3.8.2 视觉 QA 11/11。
