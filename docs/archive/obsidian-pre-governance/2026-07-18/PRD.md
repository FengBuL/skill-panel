...
顶部主导航的新增、删除或职责变化属于产品信息架构变化，需要用户确认，并同步 [[AGENTS]] 与 [[UI-SPECIFICATION]]。
...
v3.8.2 已由用户确认发布，发布代码点为 `65140b081962a0177b56c1cf14c572515f320e4e`。Dashboard 视觉 QA 已在 `421cafc9b51cb1245beb06a4a59d736b9b432d50` 恢复 11/11。`v3.8.2` tag 已创建，macOS 安装包、SHA256、源码归档和 Git 回退包已归档到 `output/releases/v3.8.2/`。当前剩余发布证据为 Windows 安装包、macOS 签名/公证、真实安装/升级验证和上一版安装包回退材料，进度见 [[CURRENT-PLAN]] 与 [[RELEASE-READINESS]]。
