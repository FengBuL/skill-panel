...
负责 Skill Library 主页面、顶部搜索筛选、Notion 风格卡片网格、右侧详情面板、卡片选中和进入完整详情。
...
| 源码路径 | 行数 | 责任 |
| --- | ---: | --- |
| `src/pages/Library/index.tsx` | 102 | 负责 Library 1:1 原型 DOM 编排、搜索、分类 pill、卡片网格、右侧详情面板、单击选中和打开完整 Detail。 |
| `src/pages/Library/Library.css` | 423 | 负责 Library 1:1 原型布局、搜索、分类 pill、卡片网格、详情面板、完整详情入口和响应式样式。 |
| `src/components/SkillCard.tsx` | 86 | 提供 Library Notion 风格 Skill 卡片，支持单击选中、双击打开 Detail、Enter 键打开 Detail 和 aria 选中状态。 |
| `src/detail/DetailPanel.tsx` | 86 | 提供 Library 右侧 340px 详情面板和“查看完整详情”入口。 |
...
- Library 右侧 DetailPanel 展示概览并提供“查看完整详情”按钮
- 卡片单击只更新选中项、aria 选中状态和右侧详情
- 卡片双击进入完整 Detail；普通 Skill 和受保护 Skill 均先停留在完整 Detail
