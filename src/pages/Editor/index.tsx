    ...
    <div className="ed-main">
      ...
      <aside className="ed-rail">
        <div className="ed-rail-section"><div className="ed-rail-label">文件</div>
          <div className="ed-file active">📄 SKILL.md</div>
          <div className="ed-file">🐍 helper.py</div>
          <div className="ed-file">⚙️ config.json</div>
          <div className="ed-file add">+ 添加文件</div>
        </div>
      ...
      <div className="ed-center">
          ...
          <div className="ed-validate">
            {validateResult ? (
              <>
                ...
                {validateResult.checks.map(c => <div key={c.id} className={`ed-val-item ${c.status}`}>{c.status==='ok'?'✓':c.status==='warn'?'⚠':'✗'} {c.label}{c.detail?`（${c.detail}）`:''}</div>)}
              </>
            ) : <div style={{fontSize:11,color:'var(--text-muted)'}}>校验中...</div>}
          </div>
        )}
        <div className="ed-fm">
          <div className="ed-fm-header">▾ Frontmatter</div>
        ...
        <div className="ed-md-toolbar">
          <button className="ed-md-tool">B</button><button className="ed-md-tool">I</button><button className="ed-md-tool">&lt;/&gt;</button><button className="ed-md-tool">🔗</button><button className="ed-md-tool">🖼</button>
          <div style={{flex:1}} />
          ...
          <button className="ed-md-tool" onClick={()=>setShowAI(!showAI)} style={{color:'var(--ai-text)'}}>✨ AI</button>
        </div>
      ...
      <div className="ed-preview">
        {showAI ? (
          <div className="ed-ai-rail">
            <div className="ed-ai-title">✨ AI 助手 <span style={{fontSize:9,color:'var(--text-faint)'}}>GLM-4</span></div>
            <div className="ed-ai-warn">⚠ 内容将发送至厂商 API（已脱敏）</div>
            <div className="ed-ai-action" onClick={()=>callAI('struct')}><span className="ed-ai-icon">🏗</span>完善结构</div>
            <div className="ed-ai-action" onClick={()=>callAI('desc')}><span className="ed-ai-icon">📝</span>优化描述</div>
            <div className="ed-ai-action" onClick={()=>callAI('polish')}><span className="ed-ai-icon">✨</span>润色正文</div>
            <div className="ed-ai-action" onClick={()=>callAI('fm')}><span className="ed-ai-icon">🏷</span>生成 frontmatter</div>
            <div className="ed-ai-action" onClick={()=>callAI('safe')}><span className="ed-ai-icon">🛡</span>安全审查</div>
