...
import { SkillExportActions } from './common/SkillExport';
import { ToastViewport, toastEventName, type ToastKind, type ToastMessage, type ToastPayload } from './common/Toast';
...
import {
  ...
  isWritableSkillSource as isWritableSkill,
  useSkillContext,
} from './stores/SkillPanelProvider';
import {
  skillSources,
  type AppSettings,
...
const virtualizedSkillThreshold = 200;
const virtualizedCategoryColumnWidth = 274;
const virtualizedCategoryRowHeight = 202;
...
function getPinyinSearchText(value: string) {
  let full = '';
  let initials = '';

  for (const character of value) {
    const pinyin = pinyinInitials[character];
    if (!pinyin) {
      full += character;
      initials += character;
      continue;
  ...
  return `${full} ${initials}`.toLocaleLowerCase();
}

function getSkillSearchIndex(skill: SkillSummary) {
  const text = `${skill.name} ${skill.description} ${skill.path}`;
  return `${text} ${getPinyinSearchText(text)}`.toLocaleLowerCase();
}
...
function filterSkills(skills: SkillSummary[], query: string) {
  ...
  return skills.filter((skill) => {
    const matchesSearch = normalizedQuery.length === 0 || getSkillSearchIndex(skill).includes(normalizedQuery);
...
function getExportFileName(name: string) {
  const safeName = name
    ...
    .replace(/^-+|-+$/g, '');
  return `${safeName || 'skill'}-skill.json`;
}

function normalizeSkillExportPackage(value: unknown): SkillExportPackage | null {
  if (!value || typeof value !== 'object') {
    return null;
  }

  const packageValue = value as Partial<SkillExportPackage>;
  if (
    typeof packageValue.name !== 'string' ||
    typeof packageValue.description !== 'string' ||
    typeof packageValue.markdown !== 'string'
  ) {
    return null;
  }

  return {
    bodyMarkdown: typeof packageValue.bodyMarkdown === 'string' ? packageValue.bodyMarkdown : packageValue.markdown,
    description: packageValue.description,
    exportedAt: typeof packageValue.exportedAt === 'string' ? packageValue.exportedAt : new Date().toISOString(),
    frontmatter: packageValue.frontmatter && typeof packageValue.frontmatter === 'object' ? packageValue.frontmatter : {},
    markdown: packageValue.markdown,
    name: packageValue.name,
    path: typeof packageValue.path === 'string' ? packageValue.path : '',
    source: packageValue.source && skillSources.includes(packageValue.source) ? packageValue.source : 'custom',
    version: 1,
  };
}
...
function VirtualizedCategorySkillGrid({
  children,
}: {
  children: ReactNode[];
}) {
  ...
  useEffect(() => {
    ...
    const updateColumnCount = () => {
      setColumnCount(Math.max(1, Math.floor(scrollElement.clientWidth / virtualizedCategoryColumnWidth)));
    };
  ...
  const rowVirtualizer = useVirtualizer({
    count: rowCount,
    estimateSize: () => virtualizedCategoryRowHeight,
  ...
  return (
    <div
      ref={scrollRef}
      className="category-card-scroll two-row-card-scroll virtualized-category-card-scroll"
      onDragOver={(event) => {
        event.preventDefault();
        event.dataTransfer.dropEffect = 'move';
      }}
    >
      <div className="virtualized-category-card-inner" style={{ height: rowVirtualizer.getTotalSize() }}>
        {rowVirtualizer.getVirtualItems().map((virtualRow) => {
          const startIndex = virtualRow.index * columnCount;
          const rowCards = children.slice(startIndex, startIndex + columnCount);

          return (
            <div
              key={virtualRow.key}
              className="virtualized-category-card-row"
              style={{
                gridTemplateColumns: `repeat(${columnCount}, minmax(0, var(--skill-card-width)))`,
...
export function SkillPanelWorkspace() {
  const {
    language,
    languageOptions,
    locale,
  ...
  const { canDeleteSkill, getSkillPermission, isReadOnlySkill, isSkillLocked, isWritableSkill } = useSkillContext();
  const [skills, setSkills] = useState<SkillSummary[]>([]);
  const [activeWorkspace, setActiveWorkspace] = useState<WorkspaceView>('library');
  ...
  const virtualLibraryEnabled = sortedFilteredSkills.length > 200;
  const virtualColumnCount = 3;
  const virtualRowHeight = libraryDensity === 'comfortable' ? 268 : 228;
  const virtualStartRow = virtualLibraryEnabled ? Math.max(0, Math.floor(libraryScrollTop / virtualRowHeight) - 2) : 0;
  const virtualVisibleRows = 10;
  const virtualStartIndex = virtualStartRow * virtualColumnCount;
  const virtualEndIndex = Math.min(sortedFilteredSkills.length, virtualStartIndex + virtualVisibleRows * virtualColumnCount);
  const visibleLibrarySkills = virtualLibraryEnabled ? sortedFilteredSkills.slice(virtualStartIndex, virtualEndIndex) : paginatedSkills;
  const virtualTopSpacerHeight = virtualLibraryEnabled ? virtualStartRow * virtualRowHeight : 0;
  const virtualBottomSpacerHeight = virtualLibraryEnabled
    ? Math.max(0, Math.ceil((sortedFilteredSkills.length - virtualEndIndex) / virtualColumnCount) * virtualRowHeight)
    : 0;
      ...
      : visibleScanOutcome === 'success'
        ...
        : visibleScanOutcome === 'partial-success'
          ...
          : visibleScanOutcome === 'failed'
            ? 'sources.failed'
            : 'sources.notScanned';
  const selectedPermission = selectedDetail ? getSkillPermission(selectedDetail, Boolean(skillLocks[selectedDetail.path])) : null;
  const selectedIsPermanentlyLocked = selectedPermission?.readonly ?? false;
  const selectedIsLocked = selectedPermission?.locked ?? false;
  const selectedCanDelete = selectedPermission?.canDelete ?? false;
  ...
  useEffect(() => {
    const handleToastEvent = (event: Event) => {
      ...
      if (!toastEvent.detail?.message) {
        return;
      }
      showToast(toastEvent.detail.kind, toastEvent.detail.message);
    };

    window.addEventListener(toastEventName, handleToastEvent);
    return () => window.removeEventListener(toastEventName, handleToastEvent);
  }, []);

  const auditOperation = async (action: string, detail: Record<string, unknown> = {}) => {
    try {
      await invoke('append_audit_log', { entry: { action, detail, timestamp: new Date().toISOString() } });
    } catch {
      // Audit logging cannot block user actions.
    }
  };
  ...
  const performEditorSave = async () => {
    ...
    try {
      ...
      void auditOperation(editorDraft.kind === 'existing' ? 'skill.update' : 'skill.create', { name: savedDetail.name, path: savedDetail.path });
      await scanSkills();
      upsertDetailIntoSkills(savedDetail);
    } catch (error) {
  ...
  const performSelectedSkillSave = async () => {
    ...
    try {
      ...
      showToast('success', t('settings.savedTitle'));
      void auditOperation('skill.update', { name: updatedDetail.name, path: updatedDetail.path });
    } catch (error) {
      setDetailErrorTitleKey('details.actionErrorTitle');
      setDetailError(error instanceof Error ? error.message : String(error));
    } finally {
  ...
  const createSkill = async () => {
    ...
    try {
      ...
      setShowCreateSkill(false);
      showToast('success', t('export.imported', { name: createdDetail.name }));
      void auditOperation('skill.create', { name: createdDetail.name, path: createdDetail.path });
      await scanSkills();
  ...
  const copySkillPath = async (path: string) => {
    ...
    try {
      ...
      showToast('success', t('library.copyPath'));
      void auditOperation('skill.copyPath', { path });
    } catch (error) {
      setDetailErrorTitleKey('details.actionErrorTitle');
      setDetailError(error instanceof Error ? error.message : String(error));
    }
  ...
  const exportSelectedSkill = () => {
    ...
    const payload: SkillExportPackage = {
      bodyMarkdown: selectedDetail.bodyMarkdown,
      description: selectedDetail.description,
      exportedAt: new Date().toISOString(),
      frontmatter: selectedDetail.frontmatter,
      markdown: buildFullMarkdownFromDetail(selectedDetail),
    ...
    link.href = url;
    link.download = getExportFileName(selectedDetail.name);
    document.body.append(link);
    link.click();
    link.remove();
    URL.revokeObjectURL(url);
    showToast('success', t('export.exportSkill'));
    void auditOperation('skill.export', { name: selectedDetail.name, path: selectedDetail.path });
  };

  const importSkillPackage = async (file: File) => {
    setDetailError(null);
    setBulkStatus(null);

    try {
      const payload = normalizeSkillExportPackage(JSON.parse(await file.text()));
      if (!payload) {
        throw new Error(t('export.invalidPackage'));
      }

      const parsed = parseEditorMarkdown(payload.markdown, payload.name, payload.description);
      const createdDetail = await invoke<SkillDetail>('create_skill', {
        input: {
      ...
      setSelectedPath(createdDetail.path);
      syncDetailForm(createdDetail);
      const message = t('export.imported', { name: createdDetail.name });
      setBulkStatus(message);
      showToast('success', message);
      void auditOperation('skill.import', { name: createdDetail.name, path: createdDetail.path });
      await scanSkills();
      upsertDetailIntoSkills(createdDetail);
    } catch (error) {
      setDetailErrorTitleKey('details.actionErrorTitle');
      setDetailError(t('export.importFailed', { message: error instanceof Error ? error.message : String(error) }));
    }
  };
  ...
  const updateSkillLock = (path: string, locked: boolean) => {
    ...
    setSkillTagContextMenu(null);
    void auditOperation('skill.lock', { locked, path });
  };
  ...
  const invertCurrentSelection = () => {
    const visiblePaths = sortedFilteredSkills.map((skill) => skill.path);
    const nextSelection = new Set<string>();

    for (const path of visiblePaths) {
      if (!selectedSkillPaths.has(path)) {
        nextSelection.add(path);
      }
    }

    setSelectedSkillPaths(nextSelection);
    setBulkStatus(t('bulk.selectedCount', { count: String(nextSelection.size) }));
  };
  ...
  const applyBulkCategory = () => {
    ...
    const message = t('bulk.updatedCount', { count: String(selectedSkillCount) });
    setBulkStatus(message);
    showToast('success', message);
    void auditOperation('bulk.category', { categoryId: bulkCategoryId, count: selectedSkillCount });
    persistUiPreferences(
      categoryColors,
      categoryLabels,
      skillTags,
  ...
  const applyBulkTag = () => {
    ...
    setBulkStatus(message);
    showToast('success', message);
    void auditOperation('bulk.tag', { count: selectedSkillCount, label });
    persistUiPreferences(
      categoryColors,
      categoryLabels,
      nextTags,
  ...
  const applyBulkColor = () => {
    ...
    setBulkStatus(message);
    showToast('success', message);
    void auditOperation('bulk.color', { color: bulkColor, count: selectedSkillCount });
    persistUiPreferences(
      categoryColors,
      categoryLabels,
      skillTags,
  ...
  const applyBulkLock = (locked: boolean) => {
    ...
    const message = t('bulk.updatedCount', { count: String(selectedSkills.filter(isWritableSkill).length) });
    setBulkStatus(message);
    showToast('success', message);
    void auditOperation('bulk.lock', { count: selectedSkillCount, locked });
    persistUiPreferences(
      categoryColors,
      categoryLabels,
      skillTags,
  ...
  const applyBulkFavorite = (favorite: boolean) => {
    ...
    setBulkStatus(message);
    showToast('success', message);
    void auditOperation('bulk.favorite', { count: selectedSkillCount, favorite });
    persistUiPreferences(
      categoryColors,
      categoryLabels,
      skillTags,
  ...
  const applyBulkArchive = (archived: boolean) => {
    ...
    setBulkStatus(message);
    showToast('success', message);
    void auditOperation(archived ? 'bulk.archive' : 'bulk.restore', { archived, count: selectedSkillCount });
    persistUiPreferences(
      categoryColors,
      categoryLabels,
      skillTags,
  ...
  const performBulkDelete = async (targetPaths: string[]) => {
    ...
    setDeleteConfirmation(null);
    const deletedMessage = t('bulk.deletedCount', { count: String(deletedPaths.size) });
    setBulkStatus(deletedMessage);
    showToast(deletedPaths.size > 0 ? 'success' : 'info', deletedMessage);
    void auditOperation('skill.delete', { count: deletedPaths.size, paths: Array.from(deletedPaths) });
  ...
  const scheduleSelectedSkillsDelete = () => {
    ...
    setDeleteConfirmation(null);
    const count = deletablePaths.length;
    const message = t('bulk.deleteScheduled', { count: String(count) });
    setBulkStatus(message);
    void auditOperation('skill.deleteScheduled', { count, paths: deletablePaths });
    let toastId = 0;
    const timerId = window.setTimeout(() => {
      dismissToast(toastId);
      setPendingDelete((current) => (current?.toastId === toastId ? null : current));
      void performBulkDelete(deletablePaths);
    }, 8000);
    toastId = showToast('info', message, {
      actionLabel: t('bulk.undoDelete'),
      durationMs: 9000,
      onAction: () => {
        window.clearTimeout(timerId);
        dismissToast(toastId);
        setPendingDelete(null);
        const undoMessage = t('bulk.deleteUndone', { count: String(count) });
        setBulkStatus(undoMessage);
        showToast('info', undoMessage);
        void auditOperation('skill.deleteUndo', { count, paths: deletablePaths });
      },
    });
    setPendingDelete({ paths: deletablePaths, timerId, toastId });
  };

  const reorderCategorySkill = (categoryId: CategoryId, targetPath: string, sourcePath = draggedSkillPath, position?: 'after' | 'before') => {
    ...
    if (!position) {
      ...
      const nextTargetIndex = nextOrder.indexOf(targetPath);
      nextOrder.splice(position === 'after' ? nextTargetIndex + 1 : nextTargetIndex, 0, sourcePath);
    }
    ...
    void auditOperation('skill.reorder', { categoryId, sourcePath, targetPath });
  };
  ...
  const restoreCategoryDefaultOrder = (categoryId: CategoryId) => {
    ...
    setCategorySkillOrder(nextCategorySkillOrder);
    persistUiPreferences(categoryColors, categoryLabels, skillTags, nextCategorySkillOrder);
    void auditOperation('skill.order.restoreDefault', { categoryId });
  };
  ...
  const {
    ...
    onReorder: reorderCategorySkill,
  });
  ...
  const detailActions = (
    <div className="detail-actions detail-actions-pinned" style={{ position: 'static' }}>
      ...
      <SkillExportActions
        canExport={Boolean(selectedDetail)}
        exportLabel={t('export.exportSkill')}
        importLabel={t('export.importSkill')}
        onExport={exportSelectedSkill}
        onImport={(file) => void importSkillPackage(file)}
      />
    </div>
  );
  const contextMenuSkill = skillTagContextMenu ? skills.find((skill) => skill.path === skillTagContextMenu.path) : null;
  const contextMenuPermission = contextMenuSkill ? getSkillPermission(contextMenuSkill, Boolean(skillLocks[contextMenuSkill.path])) : null;
  const contextMenuPermanentlyLocked = contextMenuPermission?.readonly ?? false;
  const contextMenuLocked = contextMenuPermission?.locked ?? false;
  ...
  const moveFocusedSkillCard = (direction: 1 | -1) => {
    const cards = Array.from(document.querySelectorAll<HTMLElement>('[data-skill-card-path]')).filter((card) => card.offsetParent !== null);
    if (cards.length === 0) {
      return;
    }

    const activeElement = document.activeElement instanceof HTMLElement ? document.activeElement.closest<HTMLElement>('[data-skill-card-path]') : null;
    const activeIndex = activeElement ? cards.indexOf(activeElement) : -1;
    const nextIndex = activeIndex === -1 ? (direction > 0 ? 0 : cards.length - 1) : Math.max(0, Math.min(cards.length - 1, activeIndex + direction));
    cards[nextIndex]?.focus();
  };

  useKeyboardNav({
    disabled: showSettings || showCreateSkill || Boolean(deleteConfirmation) || showSaveConfirm || showEditorSaveConfirm,
    onCommandEdit: () => {
      if (selectedDetail && !selectedIsLocked) {
        setDetailMode('edit');
      }
    },
    onCommandNew: openCreateSkillDialog,
    onCommandSave: () => {
    ...
    onMoveDown: () => moveFocusedSkillCard(1),
    onMoveUp: () => moveFocusedSkillCard(-1),
  });

  const switchWorkspace = (workspace: WorkspaceView) => {
    if (workspaceMode === 'editor' && workspace !== 'editor' && editorHasChanges && !window.confirm(t('editor.unsavedConfirm'))) {
      return;
    }
  ...
  return (
    <main className="app-shell fluid-app-shell">
      <header className="top-bar top-command-bar">
        ...
        <nav className="workspace-segmented-control" aria-label={t('workspace.primaryNavigation')}>
          {([
            ...
            <button
              ...
              aria-pressed={activeWorkspace === workspace && (workspace !== 'editor' || workspaceMode === 'editor')}
              className={activeWorkspace === workspace && (workspace !== 'editor' || workspaceMode === 'editor') ? 'active' : undefined}
      ...
      {showSettings ? (
        <ErrorBoundary variant="section" title={t('settings.title')} description={t('settings.loadErrorTitle')}>
      ...
      {workspaceMode === 'editor' ? (
        <ErrorBoundary variant="section" title={t('editor.workspace')} description={t('details.errorTitle')}>
      ...
      <section className="dashboard-grid fluid-dashboard-grid" aria-label={t('layout.dashboard')}>
        <aside className="panel sidebar fluid-sidebar-panel library-sidebar" aria-label={t('workspace.primaryNavigation')}>
          <div className="sidebar-kicker">
            <h2>{t('category.title')}</h2>
            <p>{t('category.totalSkills', { count: String(skills.length) })}</p>
          </div>
          <section className="sidebar-section topic-section" role="complementary" aria-label={t('category.title')}>
          ...
          <section className="sidebar-section library-storage-summary" aria-label={t('sources.storageTitle')}>
            ...
            <ul className="storage-path-list">
              <li>{t('sources.storagePathCount', { count: String(storagePaths.length) })}</li>
            </ul>
          </section>
        </aside>
        ...
        <section className="panel list-panel fluid-list-panel" aria-label={t('skills.title')}>
          <div className="section-heading library-heading">
              ...
              <h2>
                {activeWorkspace === 'dashboard'
                  ...
                  : activeWorkspace === 'editor'
                    ? t('workspace.editor')
                    : activeCategoryId
                      ? getCategoryLabel(buildCategoryDefinition(activeCategoryId, categoryLabels, categoryColors, categoryIcons), categoryLabels, t)
                      : activeTagLabel ?? t('sources.allSkills')}
              </h2>
            ...
            <div className="list-toolbar library-toolbar" ref={toolbarMenuRef}>
              ...
              <button type="button" className="library-attention-chip" onClick={() => jumpToHealthFilter('parse-issues')}>
                <MaterialIcon name="bar_chart" size={16} />
                <strong>{healthRuleCounts['parse-issues'] + healthRuleCounts['needs-description'] + healthRuleCounts.stale}</strong>
          ...
          {selectionMode ? (
            <div className="bulk-action-bar" role="toolbar" aria-label={t('bulk.actions')}>
              ...
              <button type="button" onClick={invertCurrentSelection}>
          ...
          {listState === 'ready' ? (
            <div className="skill-list-scroll fluid-table-region" onScroll={(event) => setLibraryScrollTop(event.currentTarget.scrollTop)}>
              <div
                className={`skill-card-grid active library-card-grid density-${libraryDensity}`}
                data-virtualized={virtualLibraryEnabled ? 'true' : undefined}
              >
                {virtualLibraryEnabled && virtualTopSpacerHeight > 0 ? (
                  <div className="virtual-spacer" style={{ height: virtualTopSpacerHeight }} aria-hidden="true" />
                ) : null}
                {visibleLibrarySkills.map((skill) => {
                  ...
                  const locked = isSkillLocked(skill, Boolean(skillLocks[skill.path]));
                  ...
                  const dragCategoryId = activeCategoryId ?? getAssignedSkillCategoryIds(skill.path, skillCategoryAssignments)[0] ?? categoryDefaults.default.id;
                  ...
                  return (
                    <SkillCard
                      ...
                      dragOffset={draggedSkillPath === skill.path ? dragOffset : releaseSkillPath === skill.path ? releaseOffset : undefined}
                      dragOverPosition={dragOverPosition}
                      isDragging={draggedSkillPath === skill.path}
                      isDragOver={dragOverSkillPath === skill.path}
                      isReleasing={releaseSkillPath === skill.path}
                      path={skill.path}
                      selected={selected}
                      style={getSkillCardStyle(skill.path, skillCardColors)}
                      onDragStart={(event) => startCategorySkillDrag(event, skill.path)}
                      onDragEnd={resetDragDrop}
                      onPointerDown={(event) => startPointerCategorySkillDrag(event, dragCategoryId, skill.path)}
                      onPointerMove={movePointerCategorySkillDrag}
                      onPointerCancel={resetDragDrop}
                      onPointerUp={(event) => finishPointerCategorySkillDrag(event, dragCategoryId, skill.path)}
                      onDragEnter={(event) => {
                        event.preventDefault();
                        event.dataTransfer.dropEffect = 'move';
                      ...
                      onDrop={(event) => {
                        event.preventDefault();
                        reorderCategorySkill(dragCategoryId, skill.path, event.dataTransfer.getData('text/plain') || draggedSkillPath);
                        resetDragDrop();
                      }}
                      onClick={() => {
                    ...
                    >
                      ...
                      <span className="skill-card-footer">
                        ...
                        <span className={favorite ? 'skill-card-star active' : 'skill-card-star'} aria-label={favorite ? t('skills.favoriteSkill') : undefined}>
                          {favorite ? '★' : '☆'}
                        </span>
                      </span>
                      <span className="library-card-actions" aria-label={t('library.cardActions')}>
                        <button
                          type="button"
                ...
                {virtualLibraryEnabled && virtualBottomSpacerHeight > 0 ? (
                  <div className="virtual-spacer" style={{ height: virtualBottomSpacerHeight }} aria-hidden="true" />
                ) : null}
              </div>
              <div className="legacy-category-grid" aria-hidden={false}>
                {categorySections.map((section) => (
                  ...
                  >
                    ...
                    {(() => {
                      const categorySkillCards = section.skills.map((skill) => {
                        const selected = selectionMode ? selectedSkillPaths.has(skill.path) : selectedPath === skill.path;
                        const customTags = skillTags[skill.path] ?? [];
                        const locked = isSkillLocked(skill, Boolean(skillLocks[skill.path]));
                        const favorite = Boolean(skillFavorites[skill.path]);
                      ...
                      return section.skills.length >= virtualizedSkillThreshold ? (
                        <VirtualizedCategorySkillGrid>{categorySkillCards}</VirtualizedCategorySkillGrid>
                      ) : (
                        <div
              ...
              <div className="pagination-controls" aria-label={t('pagination.label')}>
                <span>
                  {virtualLibraryEnabled
                    ? t('pagination.range', {
                        end: String(virtualEndIndex),
                        start: String(visibleLibrarySkills.length === 0 ? 0 : virtualStartIndex + 1),
                ...
                <div className="pagination-stepper" hidden={virtualLibraryEnabled}>
        ...
        <ErrorBoundary variant="section" title={t('details.title')} description={t('details.errorTitle')}>
          ...
          <button
            ...
            onClick={() => {
              if (contextMenuSkill) {
                ...
                setSkillTagContextMenu(null);
                void auditOperation('skill.favorite', { favorite: Boolean(nextFavorites[contextMenuSkill.path]), path: contextMenuSkill.path });
              }
            }}
          >
          ...
          <button
            ...
            onClick={() => {
              if (contextMenuSkill) {
                ...
                setSkillTagContextMenu(null);
                void auditOperation(skillArchives[contextMenuSkill.path] ? 'skill.restore' : 'skill.archive', {
                  archived: Boolean(nextArchives[contextMenuSkill.path]),
                  path: contextMenuSkill.path,
                });
              }
            }}
          >
