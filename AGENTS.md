...
1. 数据安全和发布规则。
2. 用户已经确认的验收结论。
3. 当前仓库的 `AGENTS.md`、`docs/current/product/PRD.md`、`docs/current/product/UI-SPECIFICATION.md` 和任务卡。
4. 已验收的原型交接材料。
5. 当前实现和自动化测试。
6. 历史计划、复盘和归档文档。
...
当前模块边界以 `docs/current/architecture.md` 为准。v3.8.1 模块文档历史快照位于 `docs/versions/v3.8.1/modules/`。

核心索引：

- `docs/current/architecture.md`：当前模块责任和源码边界。
- `docs/versions/v3.8.1/modules/README.md`：历史模块文档入口。
- `docs/versions/v3.8.1/modules/module-index-v3.8.1.md`：按历史模块检索代码文档。
- `docs/versions/v3.8.1/modules/source-file-index-v3.8.1.md`：按历史源码文件检索模块归属。
...
1. 在 `docs/current/architecture.md` 确认当前模块边界和源码责任。
2. 需要追溯历史设计时，在 `docs/versions/v3.8.1/modules/source-file-index-v3.8.1.md` 搜索源码文件或关键词。
3. 打开对应历史模块文档，核对后续代码变化。
4. 修改源码。

模块文档更新规则：

- 新增、移动或删除源码文件：更新 `docs/current/architecture.md` 的当前责任边界。
- 修改模块边界、对外契约或核心职责：更新 `docs/current/architecture.md` 和相关现行产品文档。
- `docs/versions/v3.8.1/modules/` 是历史快照，禁止为了匹配后续代码而改写。
- 普通文案、小样式、测试修复和小 bug 修复：可在交付说明中写“本次不涉及模块文档更新”。
...
- 说明需要迁移的代码片段或文件。
- 记录原型基线提交、原型提交和验收结论。
- 使用 `docs/current/templates/prototype-handoff.md`。
...
3. 使用 `docs/current/templates/task-card.md` 固定基线、范围、验收和风险。
...
所有 UI 和交互工作遵守 `docs/current/product/ui-style-guide.md`。
...
- 公开源码、annotated tag 和 GitHub Release 构成正式发布结果。
- macOS ARM 安装包可以作为未签名 Preview 上传，必须标注 Gatekeeper 风险和未公证状态。
- Windows 安装包只在 CI 构建成功时作为未验收 Preview 保留。
- Apple 证书、公证凭据和 Windows 人工设备不构成源码、tag 和 GitHub Release 的发布阻塞。
- 禁止读取、记录或索取证书、密钥和 secrets。
...
```text
/Users/shovy/Library/Mobile Documents/iCloud~md~obsidian/Documents/notes/skill panel项目总揽
```
...
- `docs/current/operations/DEVELOPMENT-LOG.md`：完成批次、commit、验证、证据和回退点。

产品需求变化时更新 `docs/current/product/PRD.md`，UI 决策变化时更新 `docs/current/product/UI-SPECIFICATION.md`，发布状态变化时更新 `docs/current/operations/RELEASE-READINESS.md`。截图、测试报告和安装包保存在代码项目中，Obsidian 摘要只记录路径与 commit。Obsidian 内容不自动回写仓库。
