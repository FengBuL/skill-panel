...
macOS Preview 附件：`skill-panel-v3.8.3-macos-arm64-unsigned-preview.app.zip` 和 `skill-panel-v3.8.3-macos-arm64-unsigned-preview.dmg`。

Windows Preview 附件：`skill-panel-v3.8.3-windows-x64-setup-unverified-preview.exe`。文件名中的 `unverified-preview` 是发布边界的一部分。
...
- GitHub Actions 依赖收到 Node.js 20 弃用提示，当前 runner 已强制使用 Node.js 24，构建结果通过。
